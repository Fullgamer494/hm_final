...
/**
 * Repositorio completo para gestionar causas de baja
...
/**
 * BUSCAR causa de baja por ID - VERSION OPTIONAL
 */
...
public CausaBaja save(CausaBaja causaBaja) throws SQLException {
    ...
    try (Connection conn = DatabaseConfig.getConnection();
        ...
        stmt.setString(1, causaBaja.getNombreCausaBaja());
        ...
        try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
            if (generatedKeys.next()) {
                causaBaja.setIdCausaBaja(generatedKeys.getInt(1));
            } else {
...
/**
 * ACTUALIZAR causa de baja - VERSION CON OBJETO
 */
public boolean update(CausaBaja causaBaja) throws SQLException {
    ...
    try (Connection conn = DatabaseConfig.getConnection();
        ...
        stmt.setString(1, causaBaja.getNombreCausaBaja());
        stmt.setInt(2, causaBaja.getIdCausaBaja());
...
private CausaBaja mapResultSetToCausaBaja(ResultSet rs) throws SQLException {
    ...
    causa.setIdCausaBaja(rs.getInt("id_causa_baja"));
    causa.setNombreCausaBaja(rs.getString("nombre_causa_baja"));
    return causa;
}
